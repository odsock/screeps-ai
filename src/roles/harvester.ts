...
export class Harvester extends CreepWrapper {
  public run() {
    super.run();
    ...
    else if (this.pos.findInRange(FIND_SOURCES, 1).length > 0) {
      this.fillContainer();
    }
    else {
      this.withdrawAndUpgrade();
    }
    CreepUtils.consoleLogIfWatched(this, `stumped. sitting like a lump`);
  }

  private withdrawAndUpgrade() {
  ...
  private moveToRetiree() {
    ...
    if (retiree) {
      const result = this.moveTo(retiree.pos, { visualizePathStyle: { stroke: '#ffaa00' } });
    }
    else {
  ...
  private moveToFreeContainer() {
    ...
    if (freeContainers) {
      this.moveTo(freeContainers[0], { visualizePathStyle: { stroke: '#ffaa00' } });
    }
  }
}
